...
//				exit;
//			}
				$sql=" select t, COALESCE(showup,0)  as showup from ".C('DB_PREFIX')."hours a left JOIN ";
				$sql.="(select thour, sum(showup)as showup from ";
				$sql.="(select  FROM_UNIXTIME(ad.add_time,\"%H\") as thour,showup  from ".C('DB_PREFIX')."adcount ad";
				...
				$sql.="(select  FROM_UNIXTIME(ad.add_time,\"%H\") as thour,showup  from ".C('DB_PREFIX')."adcount ad";
				$sql.=" left join ".C('DB_PREFIX')."shop sp on ad.shopid=sp.id ";
				$sql.=" where ad.add_date=DATE_ADD(CURDATE() ,INTERVAL -1 DAY) and ad.mode=1 and pid=".$this->aid;
				$sql.=" )a group by thour ) c ";
				$sql.="  on a.t=c.thour ";
				$yesterday=$db->query($sql);
				unset($sql);
				$t=date("t");
    			$sql=" select tname as t, COALESCE(showup,0)  as showup from ".C('DB_PREFIX')."day  a left JOIN";
				$sql.="( select right(ad.add_date,2) as td ,sum(showup) as showup from ".C('DB_PREFIX')."adcount  ad ";
				$sql.=" left join ".C('DB_PREFIX')."shop sp on ad.shopid=sp.id ";
				...
				$month=$db->query($sql);
			$todayjs=json_encode($today);
			$yesterdayjs=json_encode($yesterday);
			$monthjs=json_encode($month);
			$this->assign('todayjs',$todayjs);
			$this->assign('yesterdayjs',$yesterdayjs);

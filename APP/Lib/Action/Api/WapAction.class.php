...
class WapAction extends BaseApiAction{
	...
	public function index(){
		...
		$shopinfo=D('shop');
		$savepath=$shopinfo->where(array('id'=>$this->uid))->getField('save_path');
		$shopname=$shopinfo->where(array('id'=>$this->uid))->getField('shopname');
		if(cookie('feixiang')==1){
			$this->assign('feixiang',1);	
